...
class Proxy
  ...
  def method_missing(method_name, *args, &block)
    if @object.respond_to?(method_name)
      ...
      # Forward the message to the target object
      @object.send(method_name, *args, &block)
    else
      # If the target object doesn't respond to the method, raise NoMethodError
      raise NoMethodError, "undefined method `#{method_name}' for #{@object.inspect}"
    end
  end

  # Returns the list of messages sent to the proxy
  def messages
    @messages.dup # Return a copy to prevent modification
  end

  # Checks if a particular method was called
  ...
  # Needed for proper respond_to? behavior
